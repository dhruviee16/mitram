...
import { RouteTrail } from "@/components/ui/route-trail";
...
export function LiveTrackingPanel({
  ...
  const stops = parseRouteStops(routeSummary);
  ...
  return (
    <section aria-labelledby="tracking-heading" className="mt-6">
      ...
      {visible && (
        <>
          {stops.length >= 2 && (
            <RouteTrail stops={stops} updates={tripUpdates} className="-mx-4 sm:-mx-6" />
          )}
